...
`inventory/group_vars/all/user_ssh.yml` defines the managed admin `.ssh` baseline, including `authorized_keys`, optional SSH client config, and an optional known-hosts example, enforced for the example human admin account when `user.yml` enables `user_ssh`.
`inventory/group_vars/all/user_zshell.yml` defines the managed zsh login shell plus interactive `.zshrc` alias baseline enforced for the example human admin account when `user.yml` enables `user_zshell`.
`inventory/group_vars/all/user_profile.yml` defines the managed login/session profile baseline, including shared exports, PATH additions, and `.bash_profile` bridging, enforced for the example human admin account when `user.yml` enables `user_profile`.
`inventory/group_vars/all/user_directories.yml` defines the managed workspace-directory baseline, including `.local/bin`, enforced for the example human admin account when `user.yml` enables `user_directories`.
`inventory/group_vars/all/user_git.yml` defines the managed Git identity, aliases, and default behavior settings enforced for the example human admin account when `user.yml` enables `user_git`.
`inventory/group_vars/all/user_vim.yml` defines the managed `.vimrc` baseline enforced for the example human admin account when `user.yml` enables `user_vim`.
`inventory/group_vars/all/user.yml` enables the optional supplementary-group, sudo, password, SSH-access, zsh, login/profile, directory-standardization, Git-configuration, and Vim-configuration roles in the example lab, and keeps the one-run stale-sudo-drop-in cleanup toggle with the other aggregate user switches, while `inventory/group_vars/all/user_password.yml` sets a demo SHA-512 password hash for the example human admin account using the plaintext test password `password` and documents that a real host should use a Vault-managed hash instead.
