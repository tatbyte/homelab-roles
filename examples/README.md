...
`inventory/group_vars/all/user_zshell.yml` defines the managed zsh login shell plus `.zshrc` baseline enforced for the example human admin account when `user.yml` enables `user_zshell`.
