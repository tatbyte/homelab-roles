...
- `user`: Aggregates recurring human admin user configuration through explicit `include_role` ordering in `roles/user/tasks/main.yml` for `user_account` plus optional `user_groups`, optional `user_sudo`, optional `user_password`, optional `user_ssh`, optional `user_zshell`, optional `user_profile`, optional `user_directories`, and optional `user_git`, with an optional cleanup path for stale managed human-admin sudo drop-ins.
